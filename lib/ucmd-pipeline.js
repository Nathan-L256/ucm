...
function resolvePipeline(task, config) {
  ...
  let pipelineName = task.pipeline || config.defaultPipeline || DEFAULT_CONFIG.defaultPipeline;
  // "auto" resolves to the configured default pipeline
  if (pipelineName === "auto") {
    pipelineName = config.defaultPipeline || DEFAULT_CONFIG.defaultPipeline;
  }
